...
import static just.demo.visitor.Visitor.notifyGuessed;
import static just.demo.visitor.Visitor.notifyGuessing;
import static just.demo.visitor.Visitor.notifyInitial;
import static just.demo.visitor.Visitor.notifyOpeningCell;
import static just.demo.visitor.Visitor.notifyOpeningValue;
...
import just.demo.visitor.Visitor;
...
public class ComplexSolver {
  ...
  public ComplexSolver(int[][] initialValues, Visitor... visitors) {
  ...
  private int[][] solveWithGuess(Cell cell, Value value) {
    ...
    for (Cell guessCell : guessCells) {
      for (Value guessValue : guessValues) {
        ...
        try {
          solutions.add(new ComplexSolver(nextGuess, visitors).solve());
          if (solutions.size() > 1) {
            throw new MultipleSolutionsException();
          }
